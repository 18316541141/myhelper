...
namespace WebApplication1.Service
{
    ...
    /// ***模块的业务类
    /// </summary>
    [Intercept(typeof(DistributedTransactionScan))]
    public partial class IRobotQrCodePayTaskService : BaseService
    {
        ...
        {
            Repository.DistributedDelete(new IRobotQrCodePayTask {
                IRTaskID=IRTaskID
            });
        }
...
----------------------------------------------------------------------------------------------------------------------------
        ...
        public void Update(IRobotQrCodePayTask data)
        {
            ...
            {
                IRTaskID = NextId(),IROrderNo = data.IROrderNo,IRWeiXinNickName = data.IRWeiXinNickName,IRWeiXinHeaderImage = data.IRWeiXinHeaderImage,IRQrCodeImagePath = data.IRQrCodeImagePath,IRHandleState = data.IRHandleState,IRHandleMessage = data.IRHandleMessage,IRHandleTime = data.IRHandleTime,IRCreateTime = data.IRCreateTime,IRReportPicPathJson = data.IRReportPicPathJson,IRTakeMoney = data.IRTakeMoney,IRRobotId = data.IRRobotId,IRRemark = data.IRRemark,IRPushState = data.IRPushState,IRPushTime = data.IRPushTime,IRScanPayNotifyRet = data.IRScanPayNotifyRet,IRScanPayNotifyUrl = data.IRScanPayNotifyUrl,
            });
        }
----------------------------------------------------------------------------------------------------------------------------
		/// <summary>
