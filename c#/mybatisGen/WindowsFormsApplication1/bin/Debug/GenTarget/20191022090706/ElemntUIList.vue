            ...
            <el-form-item>
              <el-select size="medium" v-model="postData.下拉框字段" placeholder="下拉框" clearable>
                ...
                ....
		            ...
		            <el-form-item>
		                <el-input size="medium" v-model="postData.VarNameLike" placeholder="变量名称" clearable></el-input>
		            </el-form-item>            <el-form-item>
		                <el-input size="medium" v-model="postData.VarValueLike" placeholder="变量值" clearable></el-input>
		            </el-form-item>
		            <el-form-item>
		                <el-input size="medium" v-model="postData.VarSortIndexStart" placeholder="变量排序号" clearable></el-input>
		            </el-form-item>
		            <el-form-item>
		                <el-input size="medium" v-model="postData.VarSortIndexEnd" placeholder="变量排序号" clearable></el-input>
		            </el-form-item>
            <el-form-item>
...
------------------------------------------------------------------------------------------------------
        列表部分：
        <default-page ref="table" url="/api/GlobalVariable/page" v-bind:post-data="postData" v-bind:ret-data.sync="retData" v-bind:reduce-height="120" v-bind:checked-datas.sync="checkedDatas"
            v-bind:show-checked="true" excel-title="测试数据.xlsx" export-url="/api/GlobalVariable/export">

                    <el-table-column prop="Id" label="主键id" sortable="custom" v-bind:show-overflow-tooltip="true" width="150px"></el-table-column>
              
                    <el-table-column prop="VarSortIndex" label="变量排序号" sortable="custom" v-bind:show-overflow-tooltip="true" width="150px"></el-table-column>
              
              
              
              
                    <el-table-column prop="VarName" label="变量名称" sortable="custom" v-bind:show-overflow-tooltip="true" width="150px"></el-table-column>
              
              
                    <el-table-column prop="VarValue" label="变量值" sortable="custom" v-bind:show-overflow-tooltip="true" width="150px"></el-table-column>
		...
		<el-dialog v-bind:title="(formData.Id===''?'新增':'编辑')+'***模块'" top="25px" width="300px"
			...
			<div style="width:100%;height:400px;overflow-y:auto;">
				...
				<div class="form-body">
									...
									<el-form-item label="变量排序号">
										<el-input v-validate="'required|rangeLength:6,20|between:6,20'" data-vv-name="formData_VarSortIndex" v-model="formData.VarSortIndex" placeholder="请填写变量排序号"></el-input>
									</el-form-item>			
									<el-form-item label="变量名称">
										<el-input v-validate="'required|rangeLength:6,20|between:6,20'" data-vv-name="formData_VarName" v-model="formData.VarName" placeholder="请填写变量名称"></el-input>
									</el-form-item>			
									<el-form-item label="变量值">
										<el-input v-validate="'required|rangeLength:6,20|between:6,20'" data-vv-name="formData_VarValue" v-model="formData.VarValue" placeholder="请填写变量值"></el-input>
									</el-form-item>
				</div>		
...
export default {
    /* 抄考代码
		...
		//仅用于存放新增表单、编辑表单的数据
		formData:{
					Id:'',VarSortIndex:'',VarName:'',VarValue:'',
		},
		//仅用于存放查询参数的数据
                postData:{
		...
		mounted(){
			...
			this.$validator.localize('zh_CN',{
				custom:{
									...
									},					'formData_VarSortIndex':{
										required:'变量排序号不能为空',
										rangeLength:'变量排序号长度须在6-20个字符之间',
										between:'变量排序号的值须在6-20之间',
									},					'formData_VarName':{
										required:'变量名称不能为空',
										rangeLength:'变量名称长度须在6-20个字符之间',
										between:'变量名称的值须在6-20之间',
									},					'formData_VarValue':{
										required:'变量值不能为空',
										rangeLength:'变量值长度须在6-20个字符之间',
										between:'变量值的值须在6-20之间',
									},
				}
			});
		},
------------------------------------------------------------------------------------------------------
        ...
        导出excel
			...
			onSubmit(){
				...
				this.$validateForm(()=>{
					if(this.formData.Id===''){
						this.$post('/api/GlobalVariable/add',this.formData,function(result){
							if(result.code===0){
						...
						this.$post('/api/GlobalVariable/edit',this.formData,function(result){
							if(result.code===0){
								this.editDialog=false;
								this.$refs.table.refresh();
							}
			...
			edit(key){
				this.$get('/api/GlobalVariable/load',{Id:key},function(result){
			...
			del(key){
				this.$post('/api/GlobalVariable/del',{ Id:key}, function(result){
					this.$refs.table.refresh();
				});
			},
			...
			enable(){
				...
				this.$post('/api/GlobalVariable/changeStatus',postData,function(result){
					this.$refs.table.refresh();
				});
			},
			...
			delBatch(){
				...
				this.$post('/api/GlobalVariable/delBatch',postData,function(result){
					this.$refs.table.refresh();
				});
			}
        }
