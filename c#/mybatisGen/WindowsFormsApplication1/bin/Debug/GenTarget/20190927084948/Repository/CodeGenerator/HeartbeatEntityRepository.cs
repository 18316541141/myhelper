    ...
    public partial class HeartbeatEntityRepository : BaseRepository<HeartbeatEntity,HeartbeatEntityParams,HeartbeatEntitySetNullParams>
    {
        ...
        IQueryable<HeartbeatEntity> OrderBy(IQueryable<HeartbeatEntity> query, HeartbeatEntityParams eqArgs)
        {
			...
			if (orderBy != null)
            {
							...
							if (orderBy.Id) { return query = query.OrderBy(a => a.Id); }else 
							if (orderBy.LastHeartbeatTime) { return query = query.OrderBy(a => a.LastHeartbeatTime); }else 
							if (orderBy.RobotIp) { return query = query.OrderBy(a => a.RobotIp); }else 
							if (orderBy.Remark) { return query = query.OrderBy(a => a.Remark); }else 
            ...
            if (orderBy != null)
            {
							...
							if (orderBy.Id) { return query = query.OrderByDescending(a => a.Id); }else 
							if (orderBy.LastHeartbeatTime) { return query = query.OrderByDescending(a => a.LastHeartbeatTime); }else 
							if (orderBy.RobotIp) { return query = query.OrderByDescending(a => a.RobotIp); }else 
							if (orderBy.Remark) { return query = query.OrderByDescending(a => a.Remark); }else 
        ...
        protected override DisPageEntity<HeartbeatEntity> GetDisPageEntity(HeartbeatEntityParams paramz)
		{
            ...
            {
                List<> keys = entities.Select(a => a.Id).ToList();
				...
				disPageEntity.OrderType = true;
							if (orderBy.Id) 
							{
								...
								disPageEntity.OrderColLazy = a => a.Id;
									disPageEntity.OrderColAndKeyLazy = a => new IRobotErrorMsg { Id = a.Id };
							...
							}				if (orderBy.LastHeartbeatTime) 
							{
								...
								disPageEntity.OrderColLazy = a => a.LastHeartbeatTime;
									disPageEntity.OrderColAndKeyLazy = a => new IRobotErrorMsg { Id = a.Id, LastHeartbeatTime = a.LastHeartbeatTime };
							...
							}				if (orderBy.RobotIp) 
							{
								...
								disPageEntity.OrderColLazy = a => a.RobotIp;
									disPageEntity.OrderColAndKeyLazy = a => new IRobotErrorMsg { Id = a.Id, RobotIp = a.RobotIp };
							...
							}				if (orderBy.Remark) 
							{
								...
								disPageEntity.OrderColLazy = a => a.Remark;
									disPageEntity.OrderColAndKeyLazy = a => new IRobotErrorMsg { Id = a.Id, Remark = a.Remark };
							...
							}				if (orderBy.ExtendField) 
							{
								...
								disPageEntity.OrderColLazy = a => a.ExtendField;
									disPageEntity.OrderColAndKeyLazy = a => new IRobotErrorMsg { Id = a.Id, ExtendField = a.ExtendField };
