 ...
 * form：layui表单对象
 */
function addCustomVerify(form){
	/**
	 * layui自定义的表单校验
	 */
	form.verify({
		/*
		 * url校验，layui内置的url校验是必填的，不填也算错误；
		 * 这里的url校验可以允许不填。
		 */
		url2: function (value, item) {
			if (value.length != 0 && !/^(https?|ftp):\/\/(((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:)*@@)?(((\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5])\.(\d|[1-9]\d|1\d\d|2[0-4]\d|25[0-5]))|((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)*(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)(:\d*)?)(\/((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@@)+(\/(([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@@)*)*)?)?(\?((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@@)|[\uE000-\uF8FF]|\/|\?)*)?(#((([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(%[\da-f]{2})|[!\$&'\(\)\*\+,;=]|:|@@)|\/|\?)*)?$/i.test(value))
				return 'url格式错误。';
		},
		//移动电话格式校验
		mobile: function (value, item) {
			if (value.length != 0 && !/^1[3|4|5|8][0-9]\d{4,8}$/.test(value))
				return '手机号码格式错误！';
		},
		//长度校验
		length: function (value, item) {
			var layLength = parseInt($(item).attr('lay-length'));
			if (value.length != 0 && value.length != layLength)
				return '输入内容长度只能是' + layLength + '位。';
		},
		//长度范围校验
		rangeLength: function (value, item) {
			var layRangeLength1 = parseInt($(item).attr('lay-range-length1'));
			var layRangeLength2 = parseInt($(item).attr('lay-range-length2'));
			if (value.length != 0 && (value.length < layRangeLength1 || value.length > layRangeLength2))
				return '输入内容长度必须是' + layRangeLength1 + '-' + layRangeLength2 + '之间。';
		},
		//数字校验
		range: function (value, item) {
			var layRange1 = parseFloat($(item).attr('lay-range1'));
			var layRange2 = parseFloat($(item).attr('lay-range2'));
			var val = parseFloat(value);
			if (!(value == '')) {
				if (!isNaN(value)) {
					if (val < layRange1 || val > layRange2)
						return '输入数字范围必须是' + layRange1 + '-' + layRange2 + '之间。';
				} else
					return '输入数字范围必须是' + layRange1 + '-' + layRange2 + '之间。';
			}
		},
		...
		equalTo: function (value, item) {
			var equalTo = $(item).attr('lay-equal-to');
			if ($(equalTo).val() !== value)
				return '确认密码和输入密码不一致。';
		},
		//金钱校验
		money: function(value, item){
			var fnum=parseFloat(value)*100;
			var inum=parseInt(fnum);
			if(fnum<0 || inum!=fnum){
				return '金额最多只能保留两位小数，并且不能小于0';
			}
		},
		//快递单号校验：
		courierNum:function(value,item){
			//各大快递公司的单号正则规则
			var checkObj=[{
					"code": "ZJS",
					"id": "103",
