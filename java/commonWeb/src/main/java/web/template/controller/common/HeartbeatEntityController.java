...
import web.template.service.common.HeartbeatEntityService;
...
public class HeartbeatEntityController extends BaseController {
	 ...
	 * 
	 * 
	 * data.setRobotId(param.getRobotId());
	 * 
	 * 
	 * -------------------------------------------------------------------------
	 ...
	 * 
	 * param.setRobotId(param.getRobotId());
	 * 
	 * param.setRobotIdLike(param.getRobotIdLike());
	 * 
	 * -------------------------------------------------------------------------
	 * --------------------------------------------------- 分页查询***模块，并返回查询结果
	 * 
	 ...
	 * @return 返回***模块的查询结果
	 * 
	 * @RequestMapping("/page") public Result page(HeartbeatEntityParams
	 * param,int currentPageIndex,int pageSize){ return new
	 ...
	 * @return 返回excel
	 * 
	 * @RequestMapping("/export") public ModelAndView
	 * export(HeartbeatEntityParams param, String excelType, int
	 * currentPageIndex, int pageSize){ Map<String,Object> model=new
	 * HashMap<String,Object>(); model.put(MyExcelView.EXCEL_DATA,
	 * service.export(param, currentPageIndex, pageSize).getPageDataList());
	 * model.put(MyExcelView.EXCEL_NAME, "excel文件名称."+excelType);
	 * model.put(MyExcelView.GROUP_NAME, "组名称"); return new
	 * ModelAndView(myExcelView, model); }
	 * -------------------------------------------------------------------------
	 * ---------------------------------------------------
	 * 
	 * @RequestMapping("/import") public Result
	 * importExcel(@RequestParam("fileUpload")MultipartFile fileUpload,
	 * 其他参数...){ List<HeartbeatEntity> HeartbeatEntityList; if
	 * (fileUpload.getOriginalFilename().endsWith("xlsx")) { HeartbeatEntityList
	 * = ExcelHelper.excelXlsxToList<HeartbeatEntity>(fileUpload.InputStream); }
	 * else if (fileUpload.getOriginalFilename().endsWith("xls")) {
	 * HeartbeatEntityList =
	 * ExcelHelper.excelXlsToList<HeartbeatEntity>(fileUpload.InputStream); }
	 * service.addBatch(HeartbeatEntityList); return new Result(0,
	 * "导入成功。",null); }
	 * -------------------------------------------------------------------------
	 * ---------------------------------------------------
	 * 
	 ...
	 * @param data 新增的数据
	 * 
	 * @RequestMapping("/add") public Result add(HeartbeatEntity data){
	 ...
	 * @param datas 修改状态的数据
	 * 
	 * @RequestMapping("/changeStatus") public Result
	 * changeStatus(HeartbeatEntity datas){ return new
	 ...
	 * 
	 * @RequestMapping("/delBatch") public Result delBatch(List<HeartbeatEntity>
