package web.template.mapper.db1;
import web.template.entity.db1.codeGenerator.IRobotQrCodePayTask;
import web.template.mapper.common.BaseMapper;
import web.template.mapper.common.DistributedMapper;
import web.template.params.db1.codeGenerator.IRobotQrCodePayTaskParams;
import web.template.setNullParams.db1.codeGenerator.IRobotQrCodePayTaskSetNullParams;
/**
 * 微信商务助手二维码支付任务表
 */
public interface IRobotQrCodePayTaskMapper extends BaseMapper<IRobotQrCodePayTask,IRobotQrCodePayTaskParams,IRobotQrCodePayTaskSetNullParams>,DistributedMapper {

}